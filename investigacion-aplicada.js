document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Obtener los proyectos de la tabla 'investigacion_aplicada'
  const { data: projects, error } = await supabase
    .from('investigacion_aplicada')  // Usar la tabla investigacion_aplicada
    .select('*');
  ...
  function projectCard(p) {
    ...
    el.className = 'card';
    el.innerHTML = `
      <div class="card__avatar" aria-hidden="true">⚗️</div>
    ...
    el.addEventListener('click', () => {
      window.location.href = `formulario-aplicada.html?id=${p.id}`;
    });
  ...
  async function fetchRespuestas(projectId) {
    const [r1, r2] = await Promise.all([
      supabase.from('investigacion_aplicada_form_proyecto').select('respuestas').eq('project_id', projectId).maybeSingle(),
      supabase.from('investigacion_aplicada_form_informe').select('respuestas').eq('project_id', projectId).maybeSingle()
    ]);
...
<body>
  ...
  fab.addEventListener('click', () => { window.location.href = 'nuevo-proyecto1.html'; });
