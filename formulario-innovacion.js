    ...
    if (type === 'proyecto') {
      ...
      document.getElementById('btnSaveProyecto').addEventListener('click', async () => {
        ...
        if (ok) {
          ...
          // Calcular puntaje y porcentaje después de guardar
          const totalPreguntas = respuestas ? Object.keys(respuestas).length : 0;
          const totalSi = respuestas ? Object.values(respuestas).filter(v => v === 'si').length : 0;
          const puntaje = totalSi; // cada "si" vale 1 punto
          const porcentaje = totalPreguntas > 0 ? ((totalSi / totalPreguntas) * 100).toFixed(2) : 0;

          // Actualizar los elementos del puntaje y porcentaje en la UI
          projectScore.textContent = puntaje;
          projectPercent.textContent = `${porcentaje}%`;
