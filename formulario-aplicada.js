document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const { data: formData, error: formError } = await supabase
      .from('investigacion_aplicada_form_proyecto')
      .select('respuestas')
  ...
  async function renderForm(type, p) {
    ...
    if (type === 'proyecto') {
      ...
      const prev = await loadRespuestas('investigacion_aplicada_form_proyecto');
      ...
      document.getElementById('btnSaveProyecto').addEventListener('click', async () => {
        ...
        const ok = await saveRespuestas('investigacion_aplicada_form_proyecto', respuestas);
